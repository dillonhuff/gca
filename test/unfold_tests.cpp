...
namespace gca {
  ...
  TEST_CASE("Unfold gcodes") {
    ...
    SECTION("One subroutine call") {
      ...
      block b2;
      b2.push_back(new token('N', 3));
      block b3;
      b3.push_back(new token('G', 2));
      b3.push_back(new token('X', 3.0));
      block b4;
      b4.push_back(new token('G', 0));
      b4.push_back(new token('Y', 2.0));
      block b5;
      b5.push_back(new token('M', 30));
